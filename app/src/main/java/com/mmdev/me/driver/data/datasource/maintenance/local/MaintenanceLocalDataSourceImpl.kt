...
class MaintenanceLocalDataSourceImpl(
	...
	): SimpleResult<MaintenanceEntity> = safeCall(TAG) {
	...
	): SimpleResult<List<MaintenanceEntity>> = safeCall(TAG) {
		dao.getMaintenanceHistory(vin, limit, offset)
	}
	
	override suspend fun getRecordById(key: Long): SimpleResult<MaintenanceEntity?> =
