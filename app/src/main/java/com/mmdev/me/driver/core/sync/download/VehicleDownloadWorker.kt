...
/**
 * Worker responsible to download only Vehicles
 */

@KoinApiExtension
class VehicleDownloadWorker(
	context: Context,
	params: WorkerParameters
) : CoroutineWorker(context, params), KoinComponent {
	
	private val downloader: IVehicleDownloader by inject()
	...
	override suspend fun doWork(): Result = withContext(MyDispatchers.io()) {
		...
		if (!email.isNullOrBlank() && !vin.isNullOrBlank() && !id.isNullOrBlank()) {
			...
			val downloadTask = async { downloader.downloadSingle(email, vin).collect {  } }
